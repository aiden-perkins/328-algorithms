...
int main(int argc, char *argv[]) {
    ...
    if (argc > 1) {
        string arg = argv[1];
        vector<vector<int>> matrixA = parse_input("./StrassenAlgorithm/" + arg + "a.txt");
        vector<vector<int>> matrixB = parse_input("./StrassenAlgorithm/" + arg + "b.txt");
...
//        cout << ans << endl;
        ...
        for (int i = 0; i <= 10; i++) {
            string fileName = "./StrassenAlgorithm/" + to_string(i) + ".txt";
